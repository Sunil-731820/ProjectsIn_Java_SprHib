...
public class LeaveDetailsDAO {
	...
	public LeaveDetails[] pendingLeaves(int empId) throws ClassNotFoundException, SQLException {
		...
		List<LeaveDetails> listDeatils = new ArrayList<LeaveDetails>();
		while(res.next()) {
			...
			listDeatils.add(leaveDetails);
		}
		return listDeatils.toArray(new LeaveDetails[listDeatils.size()]);
	}
	...
	public LeaveDetails[] leaveHistory(int empId) throws SQLException, ClassNotFoundException {
		connection = ConnectionHelper.getConnection();
		...
		ResultSet res = pst.executeQuery();
		LeaveDetails leaveDetails = null;
		List<LeaveDetails> listLeaves = new ArrayList<LeaveDetails>();
		while(res.next()) {
			leaveDetails = new LeaveDetails();
			leaveDetails.setLeaveId(res.getInt("LEAVE_ID"));
			leaveDetails.setNoOfDays(res.getInt("LEAVE_NO_OF_DAYS"));
			...
			leaveDetails.setLeaveStatus(LeaveStatus.valueOf(res.getString("LEAVE_STATUS")));
			leaveDetails.setLeaveReason(res.getString("LEAVE_REASON"));
			listLeaves.add(leaveDetails);
		}
		return listLeaves.toArray(new LeaveDetails[listLeaves.size()]);
	}
